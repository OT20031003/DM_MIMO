...
if __name__ == "__main__":
    ...
    for snr in range(min_snr_sim, max_snr_sim + 1, 1): 
        ...
        # 【修正】[-1, 1] -> [0, 1] に変換してから保存
        rec_nosample_01 = torch.clamp((rec_nosample + 1.0) / 2.0, 0.0, 1.0)
        save_img_individually(rec_nosample_01, f"{opt.nosample_outdir}/mmse_snr{snr}.png")
        ...
        print(f"DEBUG: rec_proposed Range = [{rec_proposed.min().item():.4f}, {rec_proposed.max().item():.4f}]")
        
        # 【修正】[-1, 1] -> [0, 1] に変換してから保存
        rec_proposed_01 = torch.clamp((rec_proposed + 1.0) / 2.0, 0.0, 1.0)
        save_img_individually(rec_proposed_01, f"{opt.outdir}/burst_reset_snr{snr}.png")
        ...
        for k, real_batch_idx in enumerate(monitor_indices):
            ...
            for idx in range(num_steps):
                ...
                # calculate_metrics_single は [-1, 1] を期待しているのでそのまま渡す
                ...
                save_img_individually(rec_step_01, os.path.join(inter_dir, f"step_{idx:03d}.png"))
            ...
            plot_latent_change(latent_diff_history, latent_plot_path, snr, batch_idx=real_batch_idx)
        ...
        if len(all_batches_psnr_history) > 0:
            ...
            # Average Latent Change
            if len(all_batches_latent_diff_history) > 0:
                avg_latent_diff = np.mean(np.array(all_batches_latent_diff_history), axis=0)
                avg_latent_plot_path = os.path.join(channel_outdir, f"latent_change_snr{snr}_AVERAGE.png")
                plot_latent_change(avg_latent_diff, avg_latent_plot_path, snr, batch_idx="Average")
