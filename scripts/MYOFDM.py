...
def save_img_individually(img, path):
    ...
    print(f"[Saver] Saving {img.shape[0]} images to {dirname} (base: {basename})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--burst_iterations", type=int, default=1000)
    parser.add_argument("--burst_lr", type=float, default=0.00005)
    parser.add_argument("--anchor_lambda", type=float, default=0.5)
    parser.add_argument("--h_lr_max", type=float, default=0.05)
    parser.add_argument("--h_lr_min", type=float, default=0.001)
    ...
    # OFDM / MIMO Settings (デフォルトを2x2に変更)
    ...
    # [Size Care] 2ストリームでも収まるようにシンボル数を増加 (14 -> 24)
    # 256x256 image -> 2048 complex symbols needed.
    # Capacity approx: (76-11-1) * (24-2) * 2 streams = 64 * 22 * 2 = 2816 > 2048 (OK)
    num_ofdm_symbols = 24 
    ...
    print(f"Monitoring indices: {monitor_indices} (Saved to intermediate process)")
